...
<body class="h-full bg-white dark:bg-dark-900 text-gray-900 dark:text-gray-100 transition-colors duration-200" x-data="chatApp()" x-init="init()">

    <div class="flex h-full">
        <!-- ═══ Sidebar ═══ -->
        <aside class="w-72 bg-gray-50 dark:bg-dark-800 border-r border-gray-200 dark:border-gray-700/50 flex flex-col" x-show="showSidebar" x-cloak>
            <!-- Header -->
        ...
        <main class="flex-1 flex flex-col min-w-0">
            <!-- Toolbar -->
            <header class="h-14 bg-gray-50 dark:bg-dark-800 border-b border-gray-200 dark:border-gray-700/50 flex items-center px-4 gap-3">
                <button @click="showSidebar = !showSidebar" class="text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"/>
                    </svg>
                </button>
                <h2 class="text-sm text-gray-600 dark:text-gray-300 truncate" x-text="currentTitle || 'Nueva conversación'"></h2>
                <div class="ml-auto flex items-center gap-2">
                    <span x-show="lastValidation === 'valid'" class="text-xs bg-green-100 dark:bg-green-900/50 text-green-700 dark:text-green-400 px-2 py-1 rounded">
                        ✓ Migración válida
                    </span>
                    <span x-show="lastValidation === 'error'" class="text-xs bg-red-100 dark:bg-red-900/50 text-red-700 dark:text-red-400 px-2 py-1 rounded">
                        ✗ Error en migración
                    </span>
                    <!-- Theme toggle -->
                    <button @click="toggleTheme()" class="p-2 rounded-lg text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-200 dark:hover:bg-gray-700/50 transition" title="Cambiar tema">
                            ...
                            <div x-show="msg.role === 'assistant'" class="max-w-[90%]">
                                <div class="flex items-start gap-3">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <div class="prose prose-sm dark:prose-invert max-w-none" x-html="renderMarkdown(msg.content)"></div>
                                        
                                        <!-- Schema JSON -->
                                        <div x-show="msg.schema_json" class="mt-3">
                                            <details class="bg-gray-100 dark:bg-dark-700 border border-gray-200 dark:border-gray-700/50 rounded-lg">
                                                <summary class="px-4 py-2 text-sm text-blue-600 dark:text-blue-400 cursor-pointer hover:text-blue-500 dark:hover:text-blue-300">
                                                    📋 Ver esquema JSON
                                                </summary>
                                        ...
                                        <div x-show="msg.migration_sql" class="mt-3">
                                            <div class="bg-gray-100 dark:bg-dark-700 border border-gray-200 dark:border-gray-700/50 rounded-lg">
                                                <div class="flex items-center justify-between px-4 py-2 border-b border-gray-200 dark:border-gray-700/50">
                                                    <span class="text-sm text-green-600 dark:text-green-400">📝 Migración SQL</span>
